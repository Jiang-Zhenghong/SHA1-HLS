...
//        }
        ...
        //context.Message_Block[63] = 0xC0 & 0xFF;
...
#pragma HLS array_partition variable=W dim=0
    ...
    {
        //#pragma HLS unroll
//        W[t]  = ( ( (unsigned) message_array[t * 4]     ) & 0xFF ) << 24;
//        W[t] |= ( ( (unsigned) message_array[t * 4 + 1] ) & 0xFF ) << 16;
//        W[t] |= ( ( (unsigned) message_array[t * 4 + 2] ) & 0xFF ) << 8;
//        W[t] |= ( ( (unsigned) message_array[t * 4 + 3] ) & 0xFF );
    	W[t] = message_array[t];
    }
...
//    for(t = 16; t < 80; t++)
//    {
//#pragma HLS pipeline//unroll factor=4
...
#pragma HLS pipeline
    ...
    //final_context->Length_Low            = context.Length_Low;
    //final_context->Length_High           = context.Length_High;
    //final_context->Message_Block_Index   = context.Message_Block_Index;
    //final_context->Computed              = context.Computed;
    //final_context->Corrupted             = context.Corrupted;

    //for (i = 0; i < 64; i++)
    //    final_context->Message_Block[i]  = context.Message_Block[i];
    for (i = 0; i < 5; i++)
        final_Message_Digest[i] = Message_Digest[i];
}
